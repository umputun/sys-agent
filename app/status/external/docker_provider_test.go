...
func TestDockerProvider_StatusWithRequired(t *testing.T) {
	...
	{
		resp, err := p.Status(Request{Name: "d1", URL: strings.Replace(ts.URL, "http://", "tcp://", 1) + "?containers=c1:c2"})
		require.NoError(t, err)
		assert.Equal(t, 200, resp.StatusCode)
...
func TestDockerProvider_parseDockerResponse(t *testing.T) {
	...
	assert.Equal(t, "map[blah:{blah running Up 21 hours (unhealthy)} nginx:{nginx running Up 2 seconds} weather:{weather running Up 2 hours (healthy)}]", fmt.Sprintf("%v", res["containers"]))
